...
def stem_net(input, base_filter=64):
  x = Conv2D(base_filter, 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer='he_normal')(input)
  x = BatchNormalization(axis=3)(x)
  x = Activation('relu')(x)
  ...
  x = bottleneck_Block(x, base_filter * 4, with_conv_shortcut=True)
  x = bottleneck_Block(x, base_filter * 4, with_conv_shortcut=False)
  x = bottleneck_Block(x, base_filter * 4, with_conv_shortcut=False)
  x = bottleneck_Block(x, base_filter * 4, with_conv_shortcut=False)
...
def fuse_layer1(x, base_filter=32):
  x0_0 = x[0]
  ...
  x1_0 = Conv2D(base_filter * 2, 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer='he_normal')(x[0])
...
def fuse_layer2(x, base_filter=32):
  x0_0 = x[0]
  x0_1 = Conv2D(base_filter, 1, use_bias=False, kernel_initializer='he_normal')(x[1])
  x0_1 = BatchNormalization(axis=3)(x0_1)
  x0_1 = UpSampling2D(size=(2, 2), interpolation='bilinear')(x0_1)
  ...
  x1_0 = BatchNormalization(axis=3)(x1_0)
  x1_1 = x[1]
  x1_2 = Conv2D(base_filter * 2, 1, use_bias=False, kernel_initializer='he_normal')(x[2])
  x1_2 = BatchNormalization(axis=3)(x1_2)
  x1_2 = UpSampling2D(size=(2, 2), interpolation='bilinear')(x1_2)
  x1 = add([x1_0, x1_1, x1_2])

  x2_0 = Conv2D(base_filter, 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer='he_normal')(x[0])
  x2_0 = BatchNormalization(axis=3)(x2_0)
  x2_0 = Activation('relu')(x2_0)
  x2_0 = Conv2D(base_filter * 4, 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer='he_normal')(x2_0)
  x2_0 = BatchNormalization(axis=3)(x2_0)
  x2_1 = Conv2D(base_filter * 4, 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer='he_normal')(x[1])
...
def seg_hrnet(height, width, channel, classes, alpha=1.0):
  transition_filter = [
    C(32 * alpha),
    C(64 * alpha),
    C(128 * alpha),
  ...
  inputs = Input(batch_shape=(None, height, width, channel))

  x = stem_net(inputs, base_filter=C(64 * alpha))

  x = transition_layer1(x, out_filters_list=transition_filter[:2])
  x0 = make_branch1_0(x[0], out_filters=C(32 * alpha))
  x1 = make_branch1_1(x[1], out_filters=C(64 * alpha))
  x = fuse_layer1([x0, x1], base_filter=C(32 * alpha))

  x = transition_layer2(x, out_filters_list=transition_filter[:3])
  x0 = make_branch2_0(x[0], out_filters=C(32 * alpha))
  x1 = make_branch2_1(x[1], out_filters=C(64 * alpha))
  x2 = make_branch2_2(x[2], out_filters=C(128 * alpha))
  x = fuse_layer2([x0, x1, x2], base_filter=C(32 * alpha))

  x = transition_layer3(x,  out_filters_list=transition_filter[:4])
  x0 = make_branch3_0(x[0], out_filters=C(32 * alpha))
  x1 = make_branch3_1(x[1], out_filters=C(64 * alpha))
  x2 = make_branch3_2(x[2], out_filters=C(128 * alpha))
  x3 = make_branch3_3(x[3], out_filters=C(256 * alpha))
  x = fuse_layer3([x0, x1, x2, x3], base_filter=C(32 * alpha))
...
@register_model
def HRNet(height=None, width=None, channel=4, classes=12, alpha=1.0):
  """
  For ISP
  """
  transition_filter = [
  ...
  model = Model(
    inputs={
      io.dataset_element.MAI_RAW_PATCH: inputs },
    outputs={
      io.model_prediction.ENHANCE_RGB: out },
  )
